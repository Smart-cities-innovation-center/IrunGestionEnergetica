<?
	...
	$vFechaInicio = date('Y-m-d', strtotime('Monday this week', strtotime($_SESSION['k_D_diaInicio'])));
	$vDiaAnt = date('Y-m-d', strtotime('-1 day', strtotime($vFechaInicio)));	
	$vFechaFin = date('Y-m-d', strtotime('+6 day', strtotime($vFechaInicio)));

	$listaDiasSem = array("Lu","Ma","Mi","Ju","Vi","Sa","Do");	
?>
...
<!-- Fechas Inicio -->
	...
	function calcularDia(fecha, op){
		...
		if (op==1) {  // Semana anterior
			nFecha.addWeeks(-1);
		}
		if (op==2) {  // Semana siguiente
			nFecha.addWeeks(1);
		}
		...
		if (nFecha.getDay()==0) {
			...
			nDias = (nFecha.getDay()-1) * -1;
		}
	
		nFecha.addDays(nDias);
	...
	<section role="main" id="main">
		...
		<div class="with-padding" style="margin-bottom:30px;">
			...
			<p class="wrapped anthracite-gradient glossy with-mid-padding align-left" style="font-size:22px;">
				Panel de control Cuadro <? echo $vNumCuadro; ?> . Alumbrado . Lecturas . Semanal . <span class="orange"><? echo $vFechaInicio." a ".$vFechaFin; ?></span>
			</p>
			<br>
...
<!-- Pantalla principal Inicio -->
                        ...
                        <form action="lecturas_redirect.php" method="post" name="datosform" >
							<input type="hidden" id="periodo_tipo" name="periodo_tipo" value="S">
                            <div class="button-height" align="center">
                                <button type="submit" class="button glossy mid-margin-right" alt="Semana anterior" title="Semana anterior"
                                ...
                                <span class="input">
                                    ...
                                    <input type="text" name="datepicker" id="datepicker" class="input-unstyled datepicker" value="<? echo $vFechaInicio;?>"
                                ...
                                &nbsp;&nbsp;&nbsp;
                                <button type="submit" class="button glossy mid-margin-right" alt="Semana siguiente" title="Semana siguiente"
            ...
            <div class="tabs-content">
                ...
                <div id="tab-1" class="with-padding">						<!-- ************************  INICIO tab1 - Gráficas  **************************** -->
                    ...
                    while($swReg < $contDias){
						...
						$grafDato = substr($listaDias[$swReg], 0, 2)." (".$listaDiasSem[$swReg].")";
                    ...
                    $(document).ready(function() {
                        chart = new Highcharts.Chart({
                            ...
                            title: {
                                text: 'Potencias:  <? echo $vFechaInicio." a ".$vFechaFin; ?>',
                                x: 0 //center
                            },
                            subtitle: {
                                text: '',
                        ...
                        $(function () {
                            ...
                            $(document).ready(function() {
                                chart = new Highcharts.Chart({
                                    ...
                                    title: {
                                        text: 'Energías:  <? echo $vFechaInicio." a ".$vFechaFin; ?>',
                                        x: 0 //center
                                    },
                                    subtitle: {
                                        text: '',
							...
							<thead>
                                <tr>
                                    ...
                                    <?
                                        for ($i=0;$i<7;$i++) {
                                    ?>
                                            <th scope="col" class="align-center blue-gradient white">
												<? echo $listaDias[$i]." (".$listaDiasSem[$i].")"; ?>
                                            </th>
                                    <? 	} 
                                    ?>
                                    <th scope="col" class="align-center blue-gradient white">Media semanal</th>
                                    <th scope="col" class="align-center blue-gradient white">Acumulado semanal</th>
                                </tr>
                            </thead>
                            <tbody> 
                            <?
                ...
                <div id="tab-4" class="with-padding">						<!-- ************************  INICIO tab4 - Comparativa  **************************** -->
                	<br>
					<?
					$vFechaInicioComp =  date('Y-m-d', strtotime('-1 week', strtotime($vFechaInicio)));
					$vDiaAnt = date('Y-m-d', strtotime('-1 day', strtotime($vFechaInicioComp)));
					$vFechaFinComp = date('Y-m-d', strtotime('-1 week', strtotime($vFechaFin)));
					
					$resBD = mysql_query("SELECT * FROM tLecturas 
													WHERE lectEstacion='$vEstacion' 
													AND lectFecha>='$vFechaInicioComp' AND lectFecha<='$vFechaFinComp'
													ORDER BY lectFecha, lectHora"); 
					$numRegistrosReal = mysql_num_rows($resBD);					
					
					$resBDAnt = mysql_query("SELECT * FROM tLecturas 
														WHERE lectEstacion='$vEstacion' 
														AND lectFecha<'$vFechaInicioComp' 
														ORDER BY lectFecha DESC, lectHora DESC");	
					$numRegistrosSemAnterior = mysql_num_rows($resBDAnt);					
					
					if ($numRegistrosSemAnterior>0){  //Hay datos de semanas anteriores
						$rowBDAnt = mysql_fetch_array($resBDAnt);
						...
						$contDias = 0;
						$vDiaAux = strtotime($vFechaInicioComp);
						while ($vDiaAux<=strtotime($vFechaFinComp)) {
							$vDiaAuxTxt = date('Y-m-d',$vDiaAux);
						...
						$(function () {
							...
							$(document).ready(function() {
								chart = new Highcharts.Chart({
									...
									title: {
										text: 'Potencias:  <? echo "(".$vFechaInicio." a ".$vFechaFin.") vs (".$vFechaInicioComp." a ".$vFechaFinComp.")"; ?>',
										x: 0 //center
									},
									subtitle: {
										text: '',
                        ...
                        $(function () {
                            ...
                            $(document).ready(function() {
                                chart = new Highcharts.Chart({
                                    ...
                                    title: {
                                        text: 'Energía:  <? echo "(".$vFechaInicio." a ".$vFechaFin.") vs (".$vFechaInicioComp." a ".$vFechaFinComp.")"; ?>',
                                        x: 0 //center
                                    },
                                    subtitle: {
                                        text: '',
                ...
                <div align="center">
					<p class="big-message black-gradient green-color" style="width:50%; text-align:center">
						...
						<strong style="font-size:14px">No se puede realizar una comparativa ya que no hay datos de la semana anterior</strong>
					</p>
                </div>
                <br /><br /><br /><br /><br /><br />
                <? } ?>	                               
...
<?	
							...
							<div align="center">
								<p class="big-message black-gradient green-color" style="width:50%; text-align:center">
									<span class="big-message-icon icon-warning with-text color"></span>
									<strong style="font-size:14px">No ha lecturas en la semana seleccionada</strong>
								</p>
							</div>
							<br /><br /><br /><br /><br /><br />
							<?
		...
		<div id="menu-content">
            ...
            <section class="navigable">                   
                <ul class="big-menu">
                    ...
                    <li><a href="#" class="current navigable-current">Semanal</a></li>
