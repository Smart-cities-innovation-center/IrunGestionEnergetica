		...
		$(document).ready(function()
		{
			...
			$('#form-login').submit(function(event)
			{
				...
				{
					// Display message
					displayError('Introduzca su login');
					return false;
				}
				else if (pass.length === 0)
				{
					// Remove empty login message if displayed
					formBlock.clearMessages('Introduzca su login');

					// Display message
					displayError('Introduzca su password');
					return false;
				}
				else
				{
					...
					// Show progress
					displayLoading('Comprobando permisos...');
					  ...
					  $.ajax( {
					  		...
					  		{	var resp = data;
								...
								resp = resp.replace(" ", "");
					  			...
					  			if (resp == 'ok') {
					  				document.location.href = 'panel_control_principal.php';
					  			} else {
									
									formBlock.clearMessages();
									if (resp == 'loginNo') {
										//formLogin.clearMessages();
										displayError('Login no registrado!');
									} else {
										if (resp == 'passNo') {
											//formLogin.clearMessages();
											displayError('Password incorrecta!');	
										} else {
											if (resp == 'estadoNo') {
												//formLogin.clearMessages();
												displayError('Usuario sin acceso! Consulte con el Administrador.');	
											} else {
												//formLogin.clearMessages();
												displayError('Error de servidor! En estos momentos no hay acceso a la base de datos.');												
											}
										}
									}
								}
					  		...
					  		{
					  			formLogin.clearMessages();
					  			displayError('Error al conectar con el servidor, inténtelo de nuevo por favor');
					  		}
					  });
